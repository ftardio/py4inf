...
éste escriba {\\tt terminar}. Podrías escribir:
...
Si el usuario escribe {\tt terminar}, la sentencia {\tt break} hará que se
...
el usuario escribe ``terminar'', pero trata las líneas que empiezan por el carácter almohadilla
...
for valor in [3, 41, 12, 9, 74, 15]:
    cuenta = cuenta + 1
print 'Num. elementos: ', cuenta
\end{verbatim}
\afterverb
%
Ajustamos la variable {\tt cuenta} a cero antes de que el bucle comience,
...
En el cuerpo del bucle, añadimos 1 al valor actual de {\tt cuenta}
para cada uno de los valores de la lista. Mientras el bucle se está ejecutando, el
valor de {\tt cuenta} es el número de valores que se hayan visto ``hasta ese momento''.

Una vez el bucle se completa, el valor de {\tt cuenta} es el número total
...
En vez de añadir simplemente uno a la {\tt cuenta} como en el bucle previo,
...
introduzca ``finalizar''.
Una vez se haya introducido ``finalizar'', muestra por pantalla el total, la cantidad de números y la media
