...
\section{Extraer datos usando expresiones regulares}
...
que coinciden con un carácter que no sea un espacio en blanco ({\textbackslash}S). 
...
\section{Combinando búsqueda y extracción}
...
Se trata de una expresión muy rigurosa, que coincidirá más o menos con las líneas en las que
estamos interesados:
...
hemos usado el poder de las expresiones regulares para realizan tanto la búsqueda como la extracción de información
...
como sea posible antes de extraer esos dígitos. Este comportamiento ``codicioso'' el el motivo por el que obtenemos
los cinco dígitos de cada número. La librería de expresiones regulares se expande en ambas direcciones hasta que
encuentra un no-dígito, o el comienzo o el final de una línea.
...
A pesar de que esto funciona, en realidad se está generando un código bastante frágil que asume todas las líneas
están perfectamente formateadas. Hay que añadir suficiente comprobación de errores (o un gran bloque try/except)
para asegurarte de que tu programa nunca fallará cuando se encuentre con líneas incorrectamente formateadas, de
...
Podemos indicar que queremos simplemente encontrar un carácter, escapando ése carácter con
...
\afterverb
%
Dado que hemos antepuesto una contrabarra al signo dólar, ahora equivaldrá al símbolo del dolar
...
[aeiou] \newline
Coincide con un único carácter siempre que ése carácter esté en el conjunto especificado. En este ejemplo,
...
\beforeverb
\begin{verbatim}
Introduce archivo:mbox.txt 
38549.7949721

Introduce archivo:mbox-short.txt
39756.9259259
\end{verbatim}
\afterverb
%
