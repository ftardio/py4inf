...
enteros pertenzcan al tipo {\tt int}. Resulta sin embargo menos obvio
...
\index{carácter barra-baja}

Si le das a una variable un nombre no permitido, obtienes un error de sintaxis:
...
{\bf Los operadores} son símbolos especiales que reprentan cálculas, como
...
\index{Python 3.0}
\index{división hacia abajo}
\index{división de punto plotante}
\index{division!hacia abajo}
\index{division!punto flotante}

Cuando ambos operandos son enteros, el resultado es también un
entero; la división hacia abajo descarta la parte fraccionaria,
...
evalucación depende de las {\\bf reglas de precedencia}. Para los
operadores matemáticos, Python sigue las convenciones matemáticas.
El acrónimo {\bf PEMDSR} resulta un modo útil
de recordar esas reglas:

\index{paréntesis!modificar precedencia}

\begin{itemize}

\item Los {\bf P}aréntsis tienen el nivel superior de precedencia, y pueden usarse
...
\item La {\bf M}ultiplicación y la {\bf D}ivisión tienen la misma precendencia,
que es superior a la {\bf S}uma y {\bf R}esta, que también tienen el mismo
...
de que los cálculos se realicen en el orden que quieres.
...
El operador módulo resulta ser sorpendentemente útil. Por ejemplo,
...
\index{entrada por teclado}

A veces podemos querer recibir el valor de una variable del usuario,
...
un mensaje diciéndole qué queremos recibir. Se puede pasar una cadena a
\verb"raw_input", que será mostrada al usuario antes de la espera
...
en lenguaje natural qué es lo que es programa está haciendo. Estas notas reciben el nombre de
...
\beforeverb
\begin{verbatim}
porcentaje = (minutos * 100) / 60     # porcentaje de una hora
\end{verbatim}
\afterverb
%
...
los nombres largos pueden hacer que las expresiones complejas sean difíciles de leer, así
...
Mientras que sigas las sencillas reglas de nombrado de variables y evites las
palabras reservadas, tendrás mucho donde elegir para poner nombres a tus variables.
Al principio, esa variedad de posibilidades pueden resultarte confusas, tanto al leer un programa
como al escribir el tuyo propio. Por ejemplo, los tres
programas siguientes son idénticos si nos fijamos en lo que realizan,
...
un conjunto de datos. Trataremos los bucles pronto, pero por ahora tan sólo trata de encajar
su significado:
...
\item[commentario:]  Información en un programa que se pone para otros
